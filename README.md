...
The corrected manuscript is tag `paper-v2.4` (adds the two-matrices Figure 4, new subtitle, front matter trimmed, plain-language fixes; verified by an independent number check); `paper-v2.3` was the first reader's cut, and tag `paper-v2.3` (round-5 reviews integrated, reader's cut: 150-word abstract, one-line glance bullets, short result boxes with Tables 1–2, hidden-state detail in Appendix G, closing takeaways; 25 pages); `paper-v2`–`paper-v2.2` are earlier corrected drafts and `paper-v1` (fb3e3e8) the immutable pre-correction snapshot.
