...
\hypersetup{pdftitle={One Concept, Multiple Geometries: A Spelling Feature Fakes a Circle of Fifths, and Next-key Statistics, Not Co-occurrence, Predict What Language Models Do with Musical Keys},pdfauthor={Jan Kirin}}
...
\item \emph{The next-key table matches behaviour; PMI does not.} Spearman $\approx0.80$ against $0.1$--$0.54$; PMI identifies enharmonic twins, the table does not; direction is irrelevant on a co-mention matrix this symmetric (\S\ref{sec:operators}).
...
\begin{roadmapbox}
\S\ref{sec:corpus}--\S\ref{sec:alias} treat the symmetric statistic and the instrument failure; \S\ref{sec:behaviour}--\S\ref{sec:operators} contain the main result; \S\ref{sec:followups} summarizes the two follow-ups that Appendices~E and~F report in full. Appendix~A lists every retracted claim and post-hoc decision, Appendix~B the Fourier bookkeeping, Appendix~C the tables, Appendix~D the methods.
\end{roadmapbox}
...
bootstrap $P$(circle beats line) & $0.63$ & $1.00$ \\
...
Asked which key comes next, the models order the keys along the open line. The line partial \lc{} is $+0.14$…$+0.56$ across models and context families, with all templates significant under the glyph-preserving null in 15 of 16 model $\times$ family cells, while the circle partial is near zero in the three smaller models and $+0.13$…$+0.28$ in the 7B (Table~C.6). The first principal axis of the predictive matrices tracks the signed accidental count ($|\rho|=0.70$--$0.80$), and the top answers in relational families cluster one line step away, at the dominant or the subdominant. The line survives the pre-registered length-normalized scorer in every cell. It does not survive merging the enharmonic twins before scoring, in the three families where that scorer applies (\lc{} $-0.23$…$-0.01$); the older 12-key merged-target analysis keeps a positive line component (\lc{} $+0.69\to+0.46$, $+0.74\to+0.52$, $+0.79\to+0.66$, $+0.74\to+0.56$), and the two experiments must not be conflated.
\end{resultbox}
...
\section{The next-key table matches behaviour; PMI does not}\label{sec:operators}
...
\noindent\begin{minipage}{\linewidth}
\centering
\includegraphics[width=.96\linewidth,keepaspectratio]{figures/fig_matrices.pdf}
\kirinfigcaption{Figure 4.}{Two statistics of the same text, over the 15 spellings in line-of-fifths order. (a) PMI between keys (Karkada window, $L=16$): darker is more associated. The three enharmonic twin pairs (orange boxes) are among the most associated cells of all (ranks 2, 4 and 6 of 105). (b) The next-key table: Jensen--Shannon divergence between the rows $p(\cdot\mid i)$ of 40-word co-mention counts, darker is more alike. The twins are not identified: their rows are no more alike than a typical pair (ECI $0.57$), and a dark band runs along the line of fifths. Both matrices are fifths-smooth; they disagree only about whether C\fl{} and B are the same key.}
\end{minipage}
...
\par\vspace{4pt}
\noindent\begin{minipage}{\linewidth}
\kirinfigcaption{Table 2.}{Corpus operators over the 15 spellings. Twin ECI: mean percentile rank of the three enharmonic pairs (0 closest of all, 0.5 unremarkable), with bootstrap 95\% intervals where available (shard-cluster for the Karkada window, \texttt{results/corpus/wiki/cluster\_boot.txt}; document-cluster for the 40-word operators, \texttt{results/phase5/operator\_eci\_boot\_v4.txt}). Behaviour: Spearman correlation with the models' next-key distributions in the modulation family (OLMo-1B / Gemma / Qwen / OLMo-7B).}
...
Respelling ``moves the block with the glyph'' & \statusBounded & The surviving block follows the glyph, but at a third to four fifths of its strength and confounded with a large drop in name frequency (round-5 review). \\
